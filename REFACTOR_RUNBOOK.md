...
- 2026-04-30: Integrated `codex/concrete-standardization` at `0eebed7` into the active DX worktree. Kept the production Next route type import in `apps/docs/next-env.d.ts` after dev/server smoke checks exposed generated-file churn. Gates passed for the merge: `bun run format`, `bun run check`, `bun run build`, `bun run catalog:audit`, `bun run --cwd apps/docs typecheck:app` after removing `.next`, `bun run --cwd packages/concrete verify:publish`, and `bun run visual:smoke`.
