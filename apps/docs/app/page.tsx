...
import { getComponentPreviewState } from '@/component-catalog'
...
const primitiveRoleLabels: Partial<Record<PrimitiveSlug, string>> = {
	badge: 'status',
	'brand-mark': 'brand',
	bubble: 'chat',
	button: 'command',
					...
					{primitiveRegistry.map(entry => (
						<article className="homePrimitiveCard" key={entry.slug}>
							...
							<header>
								...
								<span>{primitiveRoleLabels[entry.slug] ?? entry.category}</span>
							</header>
					...
					{componentRegistry.map(entry => (
						<article className="componentFeatureCard" key={entry.slug}>
							...
							<div className="componentFeatureStage" data-component={entry.slug}>
								{renderComponentExample(entry.slug, getComponentPreviewState(entry.slug))}
							</div>
