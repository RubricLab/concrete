...
- Tokens: Control (3: `field-control`, `button-medium`, `toolbar-control`); Measure (4: `layout-grid-min`, `layout-sidebar`, `dialog-surface`, `trace-panel`); Viewport (2: `scroll-area-medium`, `diagram-canvas`); Icon (1: `icon-medium`); Avatar (1: `avatar-medium`); Track (2: `progress-track`, `slider-hit`); Data (1: `chart-height`); Media (1: `thumbnail`)
