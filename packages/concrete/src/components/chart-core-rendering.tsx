...
export function renderCartesianGrid(
	...
	return (
		<g className={concreteClassNames.chartGrid}>
			<rect
			...
			{ticks.map(tick => {
				...
				return (
					<g key={tick}>
						...
						{showYAxis ? (
							<text
								className={concreteClassNames.chartTickLabel}
								textAnchor="end"
								x={plotBox.left - 9}
								y={y + 3}
							>
			...
			{showXAxis ? (
				<line
					className={concreteClassNames.chartAxis}
					x1={plotBox.left}
					x2={plotBox.right}
					y1={plotBox.bottom}
					y2={plotBox.bottom}
			...
			{showXAxis
				? xLabels.map(label => (
						<text
							className={concreteClassNames.chartAxisLabel}
...
export function renderTarget(
	...
	return (
		<g className={concreteClassNames.chartTarget}>
...
export function getChartStateMessage(state: 'empty' | 'error' | 'loading' | 'ready'): string {
	switch (state) {
		case 'empty':
			return 'No data'
		case 'error':
...
export function getChartStateTone(
