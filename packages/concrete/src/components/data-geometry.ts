...
export function createAreaPath(points: readonly Point[], baseline: number): string {
	...
	return `${createPath(points)} L${round(lastPoint.x)},${round(baseline)} L${round(firstPoint.x)},${round(baseline)} Z`
}
...
export function createSmoothPath(points: readonly Point[]): string {
	...
	return points
		.map((point, pointIndex) => {
			...
			return `C${round(firstControlPoint.x)},${round(firstControlPoint.y)} ${round(secondControlPoint.x)},${round(secondControlPoint.y)} ${round(point.x)},${round(point.y)}`
		})
		.join(' ')
}
...
export function createSmoothAreaPath(points: readonly Point[], baseline: number): string {
	...
	return `${createSmoothPath(points)} L${round(lastPoint.x)},${round(baseline)} L${round(firstPoint.x)},${round(baseline)} Z`
}
